...
int frameRead(int width, int height, vqstatus *vqs, v4l2params *p)
{
  ...
  switch (p->io) 
  {
    case IO_METHOD_READ:
      ...
      imageProcess(p->buffers[0].start, width, height, vqs);
      break;
    case IO_METHOD_MMAP:
      CLEAR (buf);
      ...
      imageProcess(p->buffers[buf.index].start, width, height, vqs);
	...
	unsigned int i;
      ...
      imageProcess((void *) buf.m.userptr, width, height, vqs);
...
void mainLoop(int width, int height, vqstatus *vqs, int *running, v4l2params *p)
{
  ...
  while (*running)
  {
	...
	count++;
//printf("reading %d\n", count);
      if (frameRead(width, height, vqs, p))
        break;
...
void deviceInit(char *deviceName, int width, int height, v4l2params *p)
{
  ...
  switch (p->io)
  {
    case IO_METHOD_READ:
      ...
      break;
    case IO_METHOD_MMAP:
      mmapInit(deviceName, p);
      break;
    case IO_METHOD_USERPTR:
      userptrInit(fmt. fmt.pix.sizeimage, deviceName, p);
      break;
  }
}
